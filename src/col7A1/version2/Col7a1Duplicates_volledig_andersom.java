...
public class Col7a1Duplicates_volledig_andersom
{
    public static void main(String[] args) throws IOException, InvalidFormatException
    {
        FileOutputStream bos1 = new FileOutputStream(
                "/Users/charbonb/Desktop/Dec2014_col7a1/versie2/step4/col7a1info_report_andersom.xls");
        ...
        try
        {
            ...
            try
            {
                Writable writable = excelWriter1.createWritable("col7_duplicate_check", attributes);
                ...
                for (Entity omxEntity : repoOmx)
                {
                    ...
                    for (Entity infoEntity : repoInfo)
                    {
                        ...
                        if(equalCDNA && equalPubmed){
                            ...
                            result.set("debID 1", result.getString("debID 1")+"-"+infoEntity.getString("ID"));
                        }else if(equalCDNA && pubmedomxList.contains(pubmedinfo)){
                            System.out.println("test1 NEW");
                            result.set("duplicate cDNA and pubmed", "NEW!");
                            result.set("debID 1", "NEW! "+result.getString("debID 2")+"-"+infoEntity.getString("ID"));
                        }
                        if(equalP && equalPubmed){
                            System.out.println("test2");
                            result.set("duplicate p and pubmed", true);
                            result.set("debID 2", result.getString("debID 2")+"-"+infoEntity.getString("ID"));
                        }else if(equalP && pubmedomxList.contains(pubmedinfo)){
                            System.out.println("test2 NEW");
                            result.set("duplicate p and pubmed", "NEW!");
                            result.set("debID 2", "NEW! "+result.getString("debID 2")+"-"+infoEntity.getString("ID"));
                        }
                        if(equalCDNA && !equalPubmed){
                            System.out.println("test3");result.set("duplicate cDNA","deb pubmed: "+pubmedinfo);
                            result.set("debID 3", result.getString("debID 3")+"-"+infoEntity.getString("ID"));
                        }
                        if(equalP && !equalPubmed){
                            System.out.println("test4");
                            result.set("duplicate p","deb pubmed: "+pubmedinfo);
                            result.set("debID 4", result.getString("debID 4")+"-"+infoEntity.getString("ID"));
                        }
